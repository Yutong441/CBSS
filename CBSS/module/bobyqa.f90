...
!  have been found in the case ```NPT=N+6``` even with more than 100 variables.
    ...
    subroutine bobyqa (n, npt, x, xl, xu, rhobeg, rhoend, iprint, maxfun, calfun)
        ...
        integer,intent(in)                  :: npt     !! number of interpolation conditions. Its value must be in
                                                       ...
                                                       !! recommended.
        real(8),dimension(:),intent(inout) :: x       !! Initial values of the variables must be set in X(1),X(2),...,X(N). They
                                                       !! will be changed to the values that give the least calculated F.
        real(8),dimension(:),intent(in)    :: xl      !! lower bounds on x. The construction of quadratic models
                                                       !! requires XL(I) to be strictly less than XU(I) for each I. Further,
                                                       !! the contribution to a model from changes to the I-th variable is
                                                       !! damaged severely by rounding errors if XU(I)-XL(I) is too small.
        real(8),dimension(:),intent(in)    :: xu      !! upper bounds on x. The construction of quadratic models
                                                       !! requires XL(I) to be strictly less than XU(I) for each I. Further,
                                                       !! the contribution to a model from changes to the I-th variable is
                                                       !! damaged severely by rounding errors if XU(I)-XL(I) is too small.
        real(8),intent(in)                 :: rhobeg  !! RHOBEG must be set to the initial value of a trust region radius.  
        ...
        real(8),intent(in)                 :: rhoend  !! RHOEND must be set to the final value of a trust
...
!
90      if (dsq <= 1.0e-3_8*xoptsq) then
            fracsq = 0.25_8 * xoptsq
            sumpq = zero
            do k = 1, npt
                sumpq = sumpq + pq (k)
                sum = - half * xoptsq
...
430             format (/ 4 x, 'Return from BOBYQA because a trust',&
            ...
            else if (ratio <= 0.7_8) then
                delta = max (half*delta, dnorm)
            else
                delta = max (half*delta, dnorm+dnorm)
            end if
            if (delta <= 1.5_8*rho) delta = rho
!
...
680     if (rho > rhoend) then
            delta = half * rho
            ratio = rho / rhoend
            if (ratio <= 16.0_8) then
                rho = rhoend
            else if (ratio <= 250.0_8) then
...
!
        if (npt >= n+np) then
            do k = 2 * np, npt
                iw = (real(k-np, 8)-half) / real (n, 8)
                ip = k - np - iw * n
                iq = ip + iw
                if (iq > n) iq = iq - n
                ptsid (k) = real (ip, 8) + real (iq, 8) / real (np, 8) + sfrac
...
!
        ...
        do k = 1, npt
            ...
            if (k == kopt) then
                ...
                iq = real (np, 8) * ptsid (k) - real (ip*np, 8)
                if (iq > 0) then
                    iw = 1
                    if (ip == 0) iw = 2
...
260     do kpt = 1, npt
            ...
            pq (kpt) = zero
            ip = ptsid (kpt)
            iq = real (np, 8) * ptsid (kpt) - real (ip*np, 8)
            if (ip > 0) then
                xp = ptsaux (1, ip)
                xpt (kpt, ip) = xp
            end if
...
30      stepsq = zero
        ...
        if (gredsq*delsq <= 1.0e-4_8*qred*qred) go to 190
!
...
120     iterc = iterc + 1
        ...
        if (temp <= 1.0e-4_8*qred*qred) go to 190
        temp = sqrt (temp)
        do i = 1, n
            if (xbdi(i) == zero) then
...
!
        ...
        isav = 0
        redsav = zero
        iu = 17.0_8 * angbd + 3.1_8
        do i = 1, iu
            angt = angbd * real (i, 8) / real (iu, 8)
...
!
        ...
        if (isav < iu) then
            ...
            angt = angbd * (real(isav, 8)+half*temp) / real (iu, 8)
        end if
...
!  component of X to be in the interval [-1,1].
    ...
    subroutine bobyqa_test()
        ...
        real(8),parameter :: bdl    = - 1.0_8
        real(8),parameter :: bdu    = 1.0_8
        integer,parameter  :: iprint = 2
        integer,parameter  :: maxfun = 500000
        real(8),parameter :: rhobeg = 1.0e-1_8
        real(8),parameter :: rhoend = 1.0e-6_8
